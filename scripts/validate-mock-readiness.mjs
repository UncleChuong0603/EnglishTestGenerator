...
const feasible = validateReadingSeed().errors.length === 0 && coverage.p1 >= 6 && coverage.p2 >= 25 && coverage.p3Groups >= 13 && coverage.p4Groups >= 10 && coverage.p5 >= 30 && canChoose(part6,4,16) && canChoose(p7Single,10,29) && canChoose(p7Multiple,5,25);
const ready = !sourceOnly && feasible;
console.log(["FULL MOCK READINESS", sourceOnly ? "Mode: SOURCE FEASIBILITY (DATABASE_URL unavailable; activation cannot be certified)" : "Mode: PRODUCTION DATABASE", "", "Listening", `P1: ${coverage.p1} / 6`, `P2: ${coverage.p2} / 25`, `P3 groups: ${coverage.p3Groups} / 13`, `P4 groups: ${coverage.p4Groups} / 10`, "", "Reading", `P5: ${coverage.p5} / 30`, `P6: ${coverage.p6Groups} complete groups, ${coverage.p6} questions / 16`, `P7 single: ${coverage.p7SingleGroups} groups, ${coverage.p7SingleQuestions} questions; exact 10 / 29: ${canChoose(p7Single,10,29)?"YES":"NO"}`, `P7 multiple: ${coverage.p7MultipleGroups} groups, ${coverage.p7MultipleQuestions} questions; exact 5 / 25: ${canChoose(p7Multiple,5,25)?"YES":"NO"}`, "", `SOURCE FEASIBLE: ${feasible?"YES":"NO"}`, `READY: ${ready?"YES":"NO"}`].join("\n"));
