...
async function generate() {
  ...
  const generateAudio = async (item: (typeof items)[number]) => {
    ...
    if (existsSync(path) && !forceGeneration && current?.fingerprint === fingerprint) { skipped++; return; }
