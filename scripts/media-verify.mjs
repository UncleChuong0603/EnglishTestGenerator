...
if (provider !== "LOCAL" || !root || !process.env.DATABASE_URL) {
  ...
  try {
    ...
    const { rows: [{ remaining }] } = await pool.query("select count(*)::int as remaining from media_assets where storage_provider <> 'LOCAL' and status='READY'");
    ...
    console.log(`Non-LOCAL READY assets awaiting finalization: ${remaining}`);
