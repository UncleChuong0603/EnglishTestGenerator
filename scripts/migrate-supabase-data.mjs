...
async function run() {
  ...
  try {
    ...
    for (const user of sourceUsers) {
      ...
      await client.query(`insert into profiles (id,full_name,avatar_url,interface_language,explanation_language) values ($1,$2,$3,$4,$5) on conflict (id) do update set full_name=excluded.full_name,avatar_url=excluded.avatar_url,interface_language=excluded.interface_language,explanation_language=excluded.explanation_language,updated_at=now()`, [user.id, user.full_name ?? null, user.avatar_url ?? null, user.interface_language ?? "vi", user.explanation_language ?? "both"]);
      if (googleSub && verified) await client.query(`insert into auth_identities (user_id,provider,provider_account_id,provider_email) values ($1,'google',$2,$3) on conflict (provider,provider_account_id) do nothing`, [user.id, googleSub, email]);
      else if (verified) { const raw = randomBytes(32).toString("base64url"); await client.query(`insert into account_activation_tokens (user_id,token_hash,expires_at) values ($1,$2,now()+interval '24 hours')`, [user.id, tokenHash(raw)]); activationEmails.push({ to: email, url: `${process.env.APP_URL}/activate-account?token=${encodeURIComponent(raw)}` }); }
    }
