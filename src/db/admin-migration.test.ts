...
describe("Task 14 admin migration",()=>{
  it("is append-only after Task 13 and creates RBAC/audit tables",async()=>{ const journal=JSON.parse(readFileSync("drizzle/meta/_journal.json","utf8")); expect(journal.entries.at(-2).tag).toBe("0008_entitlements_usage"); expect(journal.entries.at(-1).tag).toBe("0009_admin_rbac"); const result=await database.query<{table_name:string}>("select table_name from information_schema.tables where table_schema='public'"); expect(result.rows.map(r=>r.table_name)).toEqual(expect.arrayContaining(["user_roles","admin_audit_logs"])); });
