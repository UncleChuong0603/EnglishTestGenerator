...
type CardPost = { slug: string; title: string; excerpt: string; category: string; content: string; publishedAt: Date | null; coverAlt?: string | null };
...
export function ArticleCard({ post, image, locale, featured = false }: { post: CardPost; image: string | null; locale: InterfaceLanguage; featured?: boolean }) {
  ...
  return <article className={`group overflow-hidden rounded-3xl border border-slate-200 bg-white transition-all duration-200 hover:-translate-y-0.5 hover:border-teal-300 hover:shadow-sm ${featured ? "md:grid md:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)]" : "flex flex-col"}`}>
    {image ? <Image alt={post.coverAlt || ""} className={featured ? "aspect-[16/10] h-full max-h-96 w-full object-cover" : "aspect-[16/9] w-full object-cover"} height={675} priority={featured} src={image} unoptimized width={1200} /> : featured ? <div aria-hidden="true" className="hidden min-h-72 bg-gradient-to-br from-teal-50 via-white to-slate-100 md:block" /> : null}
