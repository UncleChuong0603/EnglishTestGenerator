...
export async function LearnerNav({ locale, showPremiumIdentity = true }: { locale: InterfaceLanguage; showPremiumIdentity?: boolean }) {
  ...
  const secondary = [{ href: "/blog", label: locale === "vi" ? "Kiến thức" : "Guides" }, { href: "/ranking", label: locale === "vi" ? "Xếp hạng" : "Ranking" }, { href: "/demo-test", label: t.demoTest.shortTitle }, { href: "/full-mock", label: "Full Mock" }];
