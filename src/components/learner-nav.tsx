...
export async function LearnerNav({ locale, showPremiumIdentity = true }: { locale: InterfaceLanguage; showPremiumIdentity?: boolean }) {
  ...
  const items = [{ href: "/dashboard", label: t.navigation.dashboard }, { href: "/practice", label: t.navigation.practice }, { href: "/progress", label: t.navigation.progress }, { href: "/full-mock", label: locale === "vi" ? "Thi thử" : "Mock Tests" }];
  const review = [{ href: "/listening-lessons", label: locale === "vi" ? "Luyện nghe audio" : "Audio shadowing" }, { href: "/mistakes", label: t.mastery.title }, { href: "/vocabulary", label: locale === "vi" ? "Từ của tôi" : "My vocabulary" }];
  ...
  return <header className="learner-navigation relative left-1/2 flex min-h-14 w-[calc(100vw-2rem)] max-w-6xl -translate-x-1/2 items-center justify-between gap-2 border-b border-[#dce3d9] sm:w-[calc(100vw-3rem)] lg:min-h-16"><Link aria-label={locale === "vi" ? "Về trang giới thiệu TOEIC GYM" : "Go to TOEIC GYM public home"} className="inline-flex shrink-0 items-center gap-2 text-lg font-black tracking-tight text-[#183e2b] lg:text-xl" href="/"><Image src="/brand/toeic-gym-mark.png" alt="" width={32} height={32} />{t.common.brand}</Link><ActiveLearnerLinks items={items} review={review} secondary={secondary} label={t.navigation.mainLabel} locale={locale} account={account ? { name: account.name, avatarUrl: account.avatarUrl, premium: Boolean(showPremiumIdentity && account.isPremium), status } : null} settingsLabel={t.navigation.settings} signOutLabel={t.navigation.signOut} /></header>;
}
