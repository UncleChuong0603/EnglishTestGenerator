...
export async function LearnerNav({ locale, showPremiumIdentity = true }: { locale: InterfaceLanguage; showPremiumIdentity?: boolean }) {
  ...
  const items = [{ href: "/dashboard", label: t.navigation.dashboard }, { href: "/practice", label: t.navigation.practice }, { href: "/listening-lessons", label: locale === "vi" ? "Luyện nghe" : "Listening lessons" }, { href: "/progress", label: t.navigation.progress }, { href: "/full-mock", label: locale === "vi" ? "Thi thử" : "Mock Tests" }];
  const review = [{ href: "/mistakes", label: t.mastery.title }, { href: "/vocabulary", label: locale === "vi" ? "Từ của tôi" : "My vocabulary" }];
