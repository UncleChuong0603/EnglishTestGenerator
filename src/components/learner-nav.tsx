...
export async function LearnerNav({ locale, showPremiumIdentity = true }: { locale: InterfaceLanguage; showPremiumIdentity?: boolean }) {
  ...
  const secondary = [{ href: "/blog", label: locale === "vi" ? "Kiến thức" : "Guides" }, { href: "/ranking", label: locale === "vi" ? "Xếp hạng" : "Ranking" }, { href: "/support", label: locale === "vi" ? "Trợ giúp & phản hồi" : "Support & feedback" }];
