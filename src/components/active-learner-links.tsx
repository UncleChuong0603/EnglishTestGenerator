...
function Menu({ children, summary, active = false, className = "" }: { children: React.ReactNode; summary: React.ReactNode; active?: boolean; className?: string }) {
  ...
  return <details ref={ref} className={`group relative ${className}`}><summary aria-current={active ? "page" : undefined} className={`flex min-h-11 cursor-pointer list-none items-center justify-center gap-2 rounded-xl px-3 text-sm font-bold outline-offset-2 focus-visible:outline-2 focus-visible:outline-teal-700 ${active ? "bg-[#245a43] text-white" : "text-slate-700 hover:bg-[#e8eee5]"}`}>{summary}</summary>{children}</details>;
}
