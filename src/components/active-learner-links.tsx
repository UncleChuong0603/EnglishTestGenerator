...
export function ActiveLearnerLinks({ items, secondary, label, locale, account, settingsLabel, signOutLabel }: Props) {
  ...
  return <>
    ...
    <div className="lg:hidden">{account ? <Menu summary={<PremiumAvatar avatarUrl={account.avatarUrl} name={account.name} premium={account.premium} size="sm"/>}><div className="absolute right-0 z-40 mt-2 w-[min(19rem,calc(100vw-2rem))] rounded-2xl border border-slate-200 bg-white p-3 text-sm font-bold shadow-xl"><p className="truncate px-3 py-2">{account.name}{account.status ? ` · ${account.status}` : ""}</p>{accountContent}</div></Menu> : <Link className="inline-flex min-h-11 items-center font-bold text-teal-800" href="/settings">{settingsLabel}</Link>}</div>
    <nav aria-label={label} className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-slate-200 bg-white/95 px-1 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_24px_rgba(15,23,42,.08)] backdrop-blur lg:hidden">{mobileItems.map((item, index) => <Link aria-current={matches(path, item.href) ? "page" : undefined} className={`flex min-h-14 min-w-0 flex-col items-center justify-center rounded-lg px-1 text-[11px] font-bold ${matches(path, item.href) ? "text-teal-800" : "text-slate-600"}`} href={item.href} key={item.href}><span aria-hidden="true" className="text-lg leading-5">{["⌂", "▶", "◎", "▤"][index]}</span><span className="max-w-full truncate">{locale === "vi" ? ["Trang chủ", "Luyện tập", "Lỗi sai", "Thi thử"][index] : ["Home", "Practice", "Mistakes", "Mocks"][index]}</span></Link>)}<Menu active={mobileMoreActive} className="min-w-0" summary={<span className="flex flex-col items-center text-[11px]"><span aria-hidden="true" className="text-lg leading-5">☰</span>{moreLabel}</span>}><div className="absolute bottom-full right-2 z-40 mb-2 max-h-[70vh] w-[min(19rem,calc(100vw-1rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-3 text-sm font-bold shadow-xl">{mobileMoreItems.map(item => <Link aria-current={matches(path, item.href) ? "page" : undefined} className="block min-h-11 rounded-lg px-3 py-2.5 hover:bg-slate-100" href={item.href} key={item.href}>{item.label}</Link>)}<div className="mt-2 border-t border-slate-200 pt-2">{accountContent}</div></div></Menu></nav>
