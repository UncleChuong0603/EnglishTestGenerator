...
export function ListeningLessonForm({ lesson }: { lesson?: Lesson | null }) {
  return <form action={saveLessonAction} className="mt-6 space-y-5 rounded-2xl border bg-white p-5 sm:p-7">
    ...
    <label className="block font-bold">Audio MP3 {lesson ? "(để trống nếu giữ file cũ)" : "*"}<input accept="audio/mpeg" className="mt-1 block w-full rounded-lg border p-3" name="audio" required={!lesson} type="file" /></label>
