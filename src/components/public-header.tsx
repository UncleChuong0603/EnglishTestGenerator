...
export function PublicHeader({ locale, signedIn = false }: { locale: InterfaceLanguage; signedIn?: boolean }) {
  ...
  return <header className="border-b border-slate-200/80 bg-white/90">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-4 sm:gap-4 sm:px-6">
      ...
      <nav className="hidden items-center gap-1 text-sm font-semibold text-slate-600 lg:flex" aria-label="Public navigation"><Link className="rounded-lg px-3 py-2 hover:bg-slate-100" href="/blog">{locale === "vi" ? "Bài viết" : "Articles"}</Link><Link className="rounded-lg px-3 py-2 hover:bg-slate-100" href="/#features">{t.nav.features}</Link><Link className="rounded-lg px-3 py-2 hover:bg-slate-100" href="/#how-it-works">{t.nav.how}</Link><Link className="rounded-lg px-3 py-2 hover:bg-slate-100" href="/pricing">{t.nav.pricing}</Link><Link className="rounded-lg px-3 py-2 hover:bg-slate-100" href="/#faq">{t.nav.faq}</Link></nav>
    ...
    <nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 text-sm font-semibold text-slate-600 lg:hidden" aria-label="Public sections"><Link className="shrink-0 rounded-lg px-3 py-2" href="/blog">{locale === "vi" ? "Bài viết" : "Articles"}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#features">{t.nav.features}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#how-it-works">{t.nav.how}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/pricing">{t.nav.pricing}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#faq">{t.nav.faq}</Link></nav>
  </header>;
}
