...
export function PublicHeader({ locale, signedIn = false }: { locale: InterfaceLanguage; signedIn?: boolean }) {
  ...
  return <header className="border-b border-slate-200/80 bg-white/90">
    ...
    <nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 text-sm font-semibold text-slate-600 lg:hidden" aria-label="Public sections"><Link className="shrink-0 rounded-lg px-3 py-2 focus-visible:outline-2 focus-visible:outline-teal-700" href="/blog">{locale === "vi" ? "Kiến thức" : "Guides"}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#features">{t.nav.features}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#how-it-works">{t.nav.how}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/pricing">{t.nav.pricing}</Link><Link className="shrink-0 rounded-lg px-3 py-2" href="/#faq">{t.nav.faq}</Link></nav>
  </header>;
}
