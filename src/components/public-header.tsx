...
export function PublicHeader({ locale, signedIn = false }: { locale: InterfaceLanguage; signedIn?: boolean }) {
  ...
  return <header className="border-b border-slate-200/80 bg-white/90">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-4 py-4 sm:gap-4 sm:px-6">
      <Link className="flex shrink-0 items-center gap-2 font-black tracking-tight text-slate-950" href="/" aria-label="TOEIC Gym home"><span className="grid size-9 place-items-center rounded-xl bg-teal-700 text-sm text-white" aria-hidden="true">TP</span><span className="hidden sm:inline">TOEIC Gym</span></Link>
