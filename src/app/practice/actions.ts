...
export async function submitReadingPractice(sessionId: string, answers: SubmittedAnswer[]): Promise<SubmitResult> {
  ...
  try { await db.transaction(async (tx) => {
    ...
    await tx.insert(attemptAnswers).values(rows); await reconcileMasteryAnswers(tx, owner.userId, session.source, rows); await tx.update(practiceSessions).set({ status: "submitted", submittedAt: new Date(), scoreCorrect: correct, scoreTotal: session.questionCount }).where(and(eq(practiceSessions.id, sessionId), eq(practiceSessions.status, "in_progress")));
...
export async function submitListeningGroup(sessionId: string, groupId: string, answers: SubmittedAnswer[]): Promise<{ ok: true; review: ListeningGroupReview; complete: boolean } | { ok: false; error: "session_expired" | "submit_failed" }> {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!existing.length) { await tx.insert(attemptAnswers).values(rows); await reconcileMasteryAnswers(tx, owner.userId, session.source, rows); }
