...
import { GUEST_AUTH_RETURN_COOKIE, safeAuthContinuation, safeInternalReturnTo } from "@/lib/auth/redirect";
...
export async function signInAction(_state: AuthActionState, formData: FormData): Promise<AuthActionState> {
  ...
  try { await enforceRateLimit("login", await clientKey(parsed.data.email)); const user = await authenticatePassword(parsed.data.email, parsed.data.password); if (!user) return { ok: false, error: "Email hoặc mật khẩu không đúng." }; await createSession(user.id); try { await migrateGuestAttempts(user.id); } catch (error) { console.error("[guest:migration] deferred after password sign-in", { name: error instanceof Error ? error.name : "Unknown" }); } await recordProductEvent({eventName:"login_completed",userId:user.id,deduplicationKey:`login:${user.id}:${Math.floor(Date.now()/1_800_000)}`}); const stored = safeAuthContinuation((await cookies()).get(GUEST_AUTH_RETURN_COOKIE)?.value); const next = safeInternalReturnTo(parsed.data.next, stored ?? "/dashboard"); (await cookies()).delete(GUEST_AUTH_RETURN_COOKIE); redirect(next); }
...
export async function verifyEmailAction(formData: FormData) { const token = String(formData.get("token") ?? ""); try { await enforceRateLimit("verify_email", await clientKey(token.length >= 20 ? hashToken(token) : "invalid")); if (token.length >= 20 && await verifyEmailToken(token)) { const continuation = safeAuthContinuation((await cookies()).get(GUEST_AUTH_RETURN_COOKIE)?.value); redirect(continuation ? `/sign-in?verified=1&next=${encodeURIComponent(continuation)}` : "/sign-in?verified=1"); } } catch (error) { if (typeof error === "object" && error && "digest" in error) throw error; } redirect("/verify-email?error=invalid"); }
