...
export default async function DashboardPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 pb-24 text-slate-900 sm:px-6 sm:py-8 lg:pb-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6" aria-labelledby="plan-heading"><div className="flex items-center justify-between gap-4"><h2 className="text-xl font-black" id="plan-heading">{locale === "vi" ? "Quyền lợi của bạn" : "Your plan benefits"}</h2><Link className="font-bold text-teal-700" href="/pricing">{locale === "vi" ? "Xem các gói" : "View plans"}</Link></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">{([['TODAYS_WORKOUT', locale === 'vi' ? 'Bài hôm nay' : "Today's Workout"],['MANUAL_PRACTICE', locale === 'vi' ? 'Luyện tập' : 'Practice'],['MASTERY_REVIEW', locale === 'vi' ? 'Ôn lỗi sai' : 'Mastery Review'],['FULL_MOCK','Full Mock']] as const).map(([key,label]) => { const item = usage.entitlements[key]; return <div className="rounded-xl bg-slate-50 p-3" key={key}><strong>{label}</strong><p className="mt-1 text-slate-600">{item.type === 'UNLIMITED' ? (locale === 'vi' ? 'Không giới hạn' : 'Unlimited') : `${item.used} / ${item.limit} ${key === 'FULL_MOCK' ? (locale === 'vi' ? 'tháng này' : 'this month') : (locale === 'vi' ? 'hôm nay' : 'today')}`}</p></div>; })}</div></section>
