...
export default async function DashboardPage() {
  ...
  const recentAnswered = trend?.points.reduce((sum, point) => sum + point.answeredCount, 0) ?? 0;
  const recentCorrect = trend?.points.reduce((sum, point) => sum + point.correctCount, 0) ?? 0;
  const recentAccuracy = recentAnswered ? Math.round(recentCorrect / recentAnswered * 100) : null;
  const recentLearningDays = trend?.points.filter(point => point.answeredCount > 0).length ?? 0;
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 pb-24 text-slate-900 sm:px-6 sm:py-8 lg:pb-8"><div className="mx-auto max-w-6xl">
    ...
    {usage.effectivePlan === "PREMIUM" ? <section className="mt-8 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between" aria-labelledby="plan-heading"><div><h2 className="font-black" id="plan-heading">Premium</h2><p className="mt-1 text-sm text-slate-600">{locale === "vi" ? "Không giới hạn luyện tập, ôn lỗi sai và mock theo quyền truy cập hiện tại." : "Unlimited practice, mistake review, and mock access under the current policy."}</p></div><Link className="shrink-0 font-bold text-teal-700" href="/billing">{locale === "vi" ? "Quản lý gói" : "Manage plan"}</Link></section> : <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6" aria-labelledby="plan-heading"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-teal-700">FREE</p><h2 className="mt-1 text-xl font-black" id="plan-heading">{locale === "vi" ? "Mức sử dụng" : "Plan usage"}</h2></div><Link className="font-bold text-teal-700" href="/pricing">{locale === "vi" ? "Xem Premium" : "View Premium"}</Link></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">{([['TODAYS_WORKOUT', locale === 'vi' ? 'Bài hôm nay' : "Today's Workout"],['MANUAL_PRACTICE', locale === 'vi' ? 'Luyện tập' : 'Practice'],['MASTERY_REVIEW', locale === 'vi' ? 'Ôn lỗi sai' : 'Mastery Review'],['FULL_MOCK','Full Mock']] as const).map(([key,label]) => { const item = usage.entitlements[key]; if (item.type === "UNLIMITED") return null; const period = key === 'FULL_MOCK' ? (locale === 'vi' ? 'Đặt lại theo tháng' : 'Resets monthly') : (locale === 'vi' ? 'Đặt lại hằng ngày' : 'Resets daily'); return <UsageProgress key={key} label={label} used={item.used} limit={item.limit} period={period} />; })}</div></section>}
    ...
    {dashboardResult?.progress.attemptedCount ? <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-6" aria-labelledby="snapshot-heading"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-wider text-teal-700">{locale === "vi" ? "7 ngày gần đây" : "Last 7 days"}</p><h2 className="mt-1 text-xl font-black" id="snapshot-heading">{locale === "vi" ? "Ảnh chụp học tập" : "Learning snapshot"}</h2></div><Link className="font-bold text-teal-700" href="/progress">{locale === "vi" ? "Phân tích chi tiết" : "Detailed analytics"}</Link></div><div className="mt-5 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">{locale === "vi" ? "Độ chính xác gần đây" : "Recent accuracy"}</p><p className="mt-1 text-2xl font-black">{recentAccuracy === null ? "—" : `${recentAccuracy}%`}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">{locale === "vi" ? "Câu trong 7 ngày" : "7-day answers"}</p><p className="mt-1 text-2xl font-black">{recentAnswered}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">{locale === "vi" ? "Ngày có học" : "Learning days"}</p><p className="mt-1 text-2xl font-black">{recentLearningDays}</p></div><div className="rounded-xl bg-slate-50 p-4"><p className="text-sm text-slate-500">{locale === "vi" ? "Lỗi chưa xử lý" : "Unresolved mistakes"}</p><p className="mt-1 text-2xl font-black">{dashboardResult.unresolvedMistakes}</p></div></div>{trend ? <div className="mt-6"><TrendChart points={trend.points} title={locale === "vi" ? "Xu hướng độ chính xác 7 ngày" : "7-day accuracy trend"} emptyText={locale === "vi" ? "Cần hoạt động trong ít nhất 2 ngày để vẽ xu hướng." : "Activity on at least two days is needed for a trend."} /></div> : null}</section> : null}
