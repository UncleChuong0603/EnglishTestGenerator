...
import { SimilarityFilters } from "@/components/admin/similarity-filters";
...
export default async function Page({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [prefs, query, settings] = await Promise.all([
    getPreferences(actor.id),
    searchParams,
    getQuestionBankSettings(),
  ]);
  const part = Math.min(7, Math.max(1, Number(query.part) || 5));
  const threshold = Math.min(0.95, Math.max(0.25, Number(query.threshold) || settings.similarityThresholdPercent / 100));
  const lifecycle = ["draft", "published", "archived"].includes(query.lifecycle ?? "") ? query.lifecycle : undefined;
  const activeFilterCount = [query.part, query.lifecycle, query.threshold].filter(Boolean).length;
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#fef3c7,_transparent_34%),linear-gradient(180deg,#f8fafc,#ecfeff)] px-4 py-6 text-slate-900"><div className="mx-auto max-w-7xl"><AdminNav locale={prefs.interfaceLanguage}/>
    ...
    <AdminFilterPanel activeCount={activeFilterCount} clearHref="/admin/content/similarity" clearLabel="Đặt lại" label="Bộ lọc quét" summary={`Part ${part} · ${lifecycle ? lifecycle.toUpperCase() : "Tất cả trạng thái"} · ngưỡng ${Math.round(threshold * 100)}%`}><form className="grid gap-3 rounded-2xl border border-white/70 bg-white/90 p-4 shadow-sm backdrop-blur sm:grid-cols-3 lg:grid-cols-[1fr_1fr_1fr_auto]"><label className="text-sm font-bold">Part<select name="part" defaultValue={part} className="mt-1 block w-full rounded-lg border p-2.5">{[1,2,3,4,5,6,7].map((value)=><option key={value} value={value}>Part {value}</option>)}</select></label><label className="text-sm font-bold">Trạng thái<select name="lifecycle" defaultValue={lifecycle ?? ""} className="mt-1 block w-full rounded-lg border p-2.5"><option value="">Tất cả</option><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></label><label className="text-sm font-bold">Độ nhạy<select name="threshold" defaultValue={String(threshold)} className="mt-1 block w-full rounded-lg border p-2.5"><option value="0.48">Rộng · bắt nhiều nghi ngờ</option><option value="0.58">Cân bằng</option><option value="0.7">Chặt · chỉ các cặp rất giống</option><option value="0.85">Gần như trùng</option></select></label><button className="self-end rounded-lg bg-teal-700 px-5 py-3 font-black text-white">Quét Part {part}</button></form></AdminFilterPanel>
