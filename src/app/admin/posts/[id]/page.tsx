...
export default async function EditPost({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string}>}){
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6"><div className="mx-auto max-w-[1500px]"><AdminNav locale={prefs.interfaceLanguage}/><div className="mt-8 flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-black">Chỉnh sửa bài viết</h1><p className="mt-2 font-bold text-slate-500">{STATUS_LABELS[post.status as keyof typeof STATUS_LABELS]}</p></div><div className="flex flex-wrap gap-2"><Link className="rounded-xl border bg-white px-4 py-3 font-bold" href={`/admin/posts/${id}/preview`}>Xem trước</Link>{post.status==="PUBLISHED"?<form action={unpublishPostAction}>{hidden}<button className="rounded-xl bg-amber-600 px-4 py-3 font-bold text-white">Gỡ xuất bản</button></form>:<form action={publishPostAction}>{hidden}<button className="rounded-xl bg-emerald-700 px-4 py-3 font-bold text-white">Xuất bản bản đã lưu</button></form>}</div></div>
