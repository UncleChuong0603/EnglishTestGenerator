...
export default async function PostsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; category?: string; issues?: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-8 flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-black">Bài viết SEO</h1><p className="mt-2 text-slate-600">Bài đang trên blog và bài soạn trong CMS cùng xuất hiện ở đây.</p></div><Link className="rounded-xl bg-teal-700 px-5 py-3 font-black text-white" href="/admin/posts/new">Bài viết mới</Link></div>
    ...
    <AdminFilterPanel activeCount={activeFilterCount} clearHref="/admin/posts" clearLabel="Xóa bộ lọc" label="Bộ lọc"><form className="grid gap-3 rounded-2xl border bg-white p-4 sm:grid-cols-4" action="/admin/posts">
