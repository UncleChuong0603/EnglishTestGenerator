...
export default async function FullMockRunPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-5 text-slate-900"><MockNavigationGuard/><div className="mx-auto max-w-5xl">
    ...
    {active.expired ? <form action={finishMockSection.bind(null, runId)}><button className="mt-8 rounded-xl bg-teal-700 px-5 py-3 font-bold text-white">{run.mode === "FULL" && run.status === "LISTENING" ? "Chuyển sang Reading" : "Nộp bài"}</button></form> : <div className="mt-6 space-y-6">
      ...
      <form action={finishMockSection.bind(null, runId)}><button className="rounded-xl bg-teal-700 px-5 py-3 font-bold text-white">{run.mode === "FULL" && run.status === "LISTENING" ? "Hoàn tất Listening" : "Nộp bài"}</button></form>
    </div>}
  </div></main>;
}
