...
export default async function SettingsPage() {
  ...
  const [preferences, methods, plan, [ranking]] = await Promise.all([getPreferences(user.id), getUserAuthMethods(user.id), getEffectivePlan(user.id), db.select({ visibility: profiles.rankingVisibility }).from(profiles).where(eq(profiles.id, user.id)).limit(1)]);
  const t = getTranslations(preferences.interfaceLanguage); const vi = preferences.interfaceLanguage === "vi";
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 sm:px-6 sm:py-8"><div className="mx-auto max-w-6xl"><LearnerNav locale={preferences.interfaceLanguage} /><div className="mx-auto mt-10 grid max-w-2xl gap-6">
