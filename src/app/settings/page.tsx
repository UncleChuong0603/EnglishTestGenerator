...
import { getLearnerContext } from "@/lib/learner-context/service";

const sections = ["profile", "goal", "context", "language", "security", "privacy", "plan"] as const;
...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ section?: string }> }) {
  ...
  const [preferences, methods, account, [profile], goalResult, contextResult] = await Promise.all([
    ...
    getLearnerContext(user.id).then(context => ({ context, error: false as const })).catch(() => ({ context: null, error: true as const })),
  ]);
  ...
  const labels: Record<Section, string> = {
    context: vi ? "Thông tin học tập" : "Learning context",
  ...
  const summary: Record<Section, string> = {
    context: contextResult.context?.studyPurpose || contextResult.context?.acquisitionSource ? (vi ? "Đã cung cấp" : "Provided") : (vi ? "Chưa cung cấp" : "Not provided"),
      ...
      <div className="min-w-0 max-w-3xl">
        ...
        <div className={section ? "md:block" : "hidden md:block"}><Link className="mb-4 inline-flex min-h-11 items-center font-semibold text-teal-800 md:hidden" href="/settings">← {vi ? "Tất cả cài đặt" : "All settings"}</Link>
          <section aria-labelledby="section-title" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="text-2xl font-black" id="section-title">{labels[active]}</h2>
            ...
            {active === "context" && (contextResult.error ? <p className="mt-6 rounded-xl bg-red-50 p-4 text-sm text-red-700" role="alert">{vi ? "Không thể tải thông tin học tập." : "We couldn't load your learning context."}</p> : <ContextForm context={contextResult.context} locale={preferences.interfaceLanguage} />)}
