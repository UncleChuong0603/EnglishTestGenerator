...
export default async function ListeningLessonsPage() {
  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900"><div className="mx-auto max-w-6xl pb-20">
    ...
    <div className="mt-8">
      ...
      <p className="mt-2 max-w-3xl text-slate-600">{vi ? "Chọn một bài chia sẻ tự nhiên dài khoảng 1, 3, 5 hoặc 10 phút. Nghe, nhìn trước câu tiếp theo và luyện nói theo; không có câu hỏi hay chấm điểm." : "Choose a natural personal talk of about 1, 3, 5, or 10 minutes. Listen, preview the next sentence, and speak along without questions or scoring."}</p>
    </div>

    <ShadowingPlayer clips={clips} locale={prefs.interfaceLanguage} />

    <section aria-labelledby="talks-heading" className="mt-12">
      <div><h2 className="text-2xl font-black" id="talks-heading">{vi ? "Bài nghe theo thời lượng" : "Talks by length"}</h2><p className="mt-1 text-slate-600">{vi ? "Mỗi bản có mở đầu và kết thúc riêng để bạn nghe trọn một câu chuyện." : "Each version has its own opening and ending, so you can hear a complete story."}</p></div>
      <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{listeningTalks.map(talk => <Link className="rounded-2xl border bg-white p-6 hover:border-teal-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-700" href={`/listening-lessons/talk/${talk.slug}`} key={talk.slug}><span className="text-sm font-black uppercase tracking-wide text-teal-700">{talk.minutes} {vi ? "phút" : talk.minutes === 1 ? "minute" : "minutes"} · Audio</span><h3 className="mt-3 text-xl font-black">{vi ? talk.titleVi : talk.titleEn}</h3><p className="mt-2 text-slate-600">{vi ? talk.descriptionVi : talk.descriptionEn}</p><span className="mt-4 inline-block font-bold text-teal-800">{vi ? "Nghe bài này →" : "Listen to this talk →"}</span></Link>)}</div>
    </section>
  </div></main>;
}
