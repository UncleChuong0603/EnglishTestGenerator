...
export default async function PricingPage() {
  ...
  const preview = user ? await getPremiumPreview(user.id) : null;
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {account?.isPremium ? (
        ...
        <aside className="mx-auto mt-8 max-w-5xl rounded-2xl border border-slate-300 bg-white px-5 py-4">
          <strong>
      ...
      <PricingSection
        ...
        startHref={user ? "/dashboard" : "/sign-in?next=/pricing"}
