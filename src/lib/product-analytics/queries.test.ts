...
describe("analytics calculations", () => {
  ...
  it("exports an Excel-friendly long-form CSV without personal data", () => {
    const csv = productAnalyticsCsv(snapshot({ events: { landing_viewed: 10 }, signups: 2, activePremiumUsers: 1 }), "7d", "2026-09-20T00:00:00.000Z");
    expect(csv).toContain("section,metric,value,sample,period,generated_at");
    expect(csv).toContain("overview,signups,2,,7d");
    expect(csv).toContain("premium,active_premium_users,1,,7d");
