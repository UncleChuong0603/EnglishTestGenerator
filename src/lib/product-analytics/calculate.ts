...
export function challengeFunnelRows(counts: { viewed: number; started: number; completed: number; signup: number; firstWorkout: number }) {
  const values = [counts.viewed, counts.started, counts.completed, counts.signup, counts.firstWorkout];
  return values.map((count, index) => ({ count, conversion: index && values[index - 1] >= 20 ? Math.round(count / values[index - 1] * 1000) / 10 : null, denominator: index ? values[index - 1] : null }));
}
