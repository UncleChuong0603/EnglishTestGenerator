...
describe.each([3, 4] as const)("Listening Part %i group eligibility", (part) => {
  ...
  it("keeps unresolved template tokens out of a published group", () => { const input = validGroup(part); input.questions[1].text = "What will ${name} do?"; expect(validateListeningGroupEligibility(input)).toEqual({ eligible: false, reason: "UNRESOLVED_TEMPLATE" }); });
});
