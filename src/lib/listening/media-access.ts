...
export async function createAuthorizedListeningMediaUrl(input: { userId: string; sessionId: string; questionId: string; assetId: string }, storage: MediaStorage) {
  const rows = await db.select({ storageKey: mediaAssets.storageKey }).from(practiceSessions)
    ...
    .where(and(eq(practiceSessions.id, input.sessionId), eq(practiceSessions.userId, input.userId), eq(practiceSessionQuestions.questionId, input.questionId), eq(questionGroupMedia.mediaAssetId, input.assetId), eq(practiceSessions.skillArea, "LISTENING"), eq(mediaAssets.status, "READY"), eq(mediaAssets.accessScope, "CONTENT"))).limit(1);
