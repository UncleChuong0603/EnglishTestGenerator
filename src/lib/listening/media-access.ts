...
export async function createAuthorizedListeningMediaUrl(input: { userId: string | null; guestOwnerHash: string | null; sessionId: string; questionId?: string; groupId?: string; assetId: string }, storage: MediaStorage) {
  ...
  const rows = await db.select({ storageKey: mediaAssets.storageKey }).from(practiceSessions)
    ...
    .where(and(eq(practiceSessions.id, input.sessionId), input.userId ? eq(practiceSessions.userId, input.userId) : and(eq(practiceSessions.guestOwnerHash, input.guestOwnerHash!), gt(practiceSessions.expiresAt, new Date())), input.groupId ? eq(practiceSessionQuestions.passageSetId, input.groupId) : eq(practiceSessionQuestions.questionId, input.questionId!), eq(questionGroupMedia.mediaAssetId, input.assetId), eq(practiceSessions.skillArea, "LISTENING"), eq(mediaAssets.status, "READY"), eq(mediaAssets.accessScope, "CONTENT"))).limit(1);
