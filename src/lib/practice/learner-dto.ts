...
export function toLearnerPracticeQuestion(source: LearnerQuestionSource): PracticeQuestion {
  if (![1, 2, 3, 4, 5, 6, 7].includes(source.toeicPart)) throw new Error("UNSUPPORTED_LEARNER_QUESTION_PART");
  const spokenOnly = source.toeicPart <= 2;
  return { id: source.id, number: source.displayOrder, part: source.toeicPart as PracticeQuestion["part"], text: spokenOnly ? "" : source.questionText, skill: source.skill, subSkill: source.subSkill, passageSetId: source.passageSetId, options: source.options.map((option) => ({ id: option.id, key: option.optionKey, text: spokenOnly ? "" : option.optionText })) };
}
