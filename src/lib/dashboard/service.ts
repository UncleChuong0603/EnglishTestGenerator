...
export async function getDashboardData(userId: string): Promise<DashboardData> {
  ...
  return { progress, recommendation, recommendDiagnostic: shouldRecommendDiagnostic(progress, completed), activeDiagnosticId: active[0]?.id ?? null, unresolvedMistakes: mistakes.unresolved };
}
