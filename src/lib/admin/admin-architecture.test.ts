...
describe("Task 14 admin security architecture",()=>{
  ...
  it("runs server-only operator CLIs under the React server condition",()=>{ const scripts=JSON.parse(readFileSync("package.json","utf8")).scripts; for(const name of ["admin:inspect","admin:grant","admin:revoke","plan:inspect","plan:grant","plan:revoke"]) expect(scripts[name]).toContain("--conditions=react-server"); });
});
