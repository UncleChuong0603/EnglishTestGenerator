...
function reviewConditions(filters: ReviewFilters) {
  ...
  if (filters.search?.trim()) { const q = `%${filters.search.trim().slice(0, 200)}%`; conditions.push(or(ilike(passageSets.title, q), sql`exists(select 1 from questions qx where qx.passage_set_id=${passageSets.id} and qx.question_text ilike ${q})`)!); }
