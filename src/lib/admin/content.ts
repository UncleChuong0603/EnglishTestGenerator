...
export async function getContentOverview() {
  ...
  // Read one row per group. The blueprint only needs counts and unique identities.
  const units: MockUnit[] = unitRows.map((row) => ({
    id: row.id,
    part: row.part as MockUnit["part"],
    setType: (row.part === 1 ? "photographs" : row.part === 2 ? "question_response" : row.part === 3 ? "conversation" : row.part === 4 ? "talk" : row.part === 5 ? "standalone" : row.setType) as MockUnit["setType"],
    questionIds: Array.from({ length: Number(row.questionCount) }, (_, index) => `${row.id}:${index}`),
  }));
  const listeningReady = Boolean(assembleListeningMock(units));
  const readingReady = Boolean(assembleReadingMock(units));
  const form = assembleFullMock(units);
  const countPart = (part: number) => units.filter((unit) => unit.part === part);
  const p7s = units.filter((unit) => unit.part === 7 && unit.setType === "single");
  const p7m = units.filter((unit) => unit.part === 7 && unit.setType !== "single");
  return {
    lifecycle: Object.fromEntries(lifecycle.map((row) => [row.status, Number(row.value)])),
    ready: Boolean(form), listeningReady, readingReady,
    listening: {
      p1: countPart(1).length, p2: countPart(2).length,
      p3Groups: countPart(3).filter((unit) => unit.questionIds.length === 3).length,
      p3Questions: countPart(3).flatMap((unit) => unit.questionIds).length,
      p4Groups: countPart(4).filter((unit) => unit.questionIds.length === 3).length,
      p4Questions: countPart(4).flatMap((unit) => unit.questionIds).length,
    },
    reading: {
      p5: countPart(5).flatMap((unit) => unit.questionIds).length,
      p6Groups: countPart(6).filter((unit) => unit.questionIds.length === 4).length,
      p6Questions: countPart(6).flatMap((unit) => unit.questionIds).length,
      p7SingleGroups: p7s.length, p7SingleQuestions: p7s.flatMap((unit) => unit.questionIds).length,
      p7MultipleGroups: p7m.length, p7MultipleQuestions: p7m.flatMap((unit) => unit.questionIds).length,
      p7SingleFeasible: Boolean(assembleReadingMock([...units.filter((unit) => unit.part !== 7), ...p7s, ...p7m])),
      p7MultipleFeasible: readingReady,
    },
  };
}
...
function reviewConditions(filters: ReviewFilters) {
  ...
  if (filters.search?.trim()) conditions.push(contentSearch(filters.search));
