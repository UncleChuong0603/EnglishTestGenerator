...
describe("Task 15 admin content architecture",()=>{
  ...
  it("authorizes every mutation and delegates upload to configured MediaStorage",()=>{expect(actions).toContain('actor("CONTENT_MANAGE")');expect(actions).toContain('actor("MEDIA_MANAGE")');expect(actions).toContain("ingestMedia");expect(actions).toContain("createMediaStorage");});
