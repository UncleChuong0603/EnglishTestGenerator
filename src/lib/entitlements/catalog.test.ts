...
describe("entitlement catalog and Vietnam usage windows", () => {
  ...
  it("keeps useful Free analytics while Premium adds depth", () => {
    expect(getPlanCapabilities("FREE")).toMatchObject({ historyWindowDays: 30, canUseAdvancedAnalytics: false, canUseSkillBreakdown: false });
    expect(getPlanCapabilities("PREMIUM")).toMatchObject({ historyWindowDays: 90, canUseAdvancedAnalytics: true, canUseSkillBreakdown: true });
  });
});
