...
describe("entitlement enforcement architecture", () => {
  ...
  it("uses the same active Premium predicate for display and quota bypass", () => {
    expect(entitlement).toContain("function activePremiumMembership");
    expect(entitlement).toContain('eq(userPlanMemberships.planKey, "PREMIUM")');
    expect(entitlement.match(/activePremiumMembership\(/g)?.length).toBeGreaterThanOrEqual(6);
  });
