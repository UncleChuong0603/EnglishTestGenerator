...
export async function registerPasswordUser(emailInput: string, password: string) {
  const email = emailInput.trim(); const emailNormalized = normalizeEmail(email); const rawToken = createToken();
  const user = await db.transaction(async (tx) => {
    const existing = await tx.select({ id: users.id }).from(users).where(eq(users.emailNormalized, emailNormalized)).limit(1);
    if (existing.length) return null;
    const [created] = await tx.insert(users).values({ email, emailNormalized, passwordHash: await hashPassword(password) }).returning({ id: users.id, email: users.email });
...
export async function authenticatePassword(email: string, password: string) {
  ...
  if (!user?.passwordHash || !(await verifyPassword(user.passwordHash, password)) || user.status !== "active" || !user.emailVerifiedAt) return null;
...
export async function requestPasswordReset(emailInput: string) {
  const [user] = await db.select().from(users).where(eq(users.emailNormalized, normalizeEmail(emailInput))).limit(1);
  if (!user?.passwordHash || user.status === "disabled") return;
  const rawToken = createToken();
  await db.transaction(async (tx) => {
...
export async function resetPassword(rawToken: string, password: string) {
  return db.transaction(async (tx) => {
    ...
    const now = new Date();
    await tx.update(users).set({ passwordHash: await hashPassword(password), status: "active", emailVerifiedAt: sql`coalesce(${users.emailVerifiedAt}, now())`, updatedAt: now }).where(eq(users.id, token.userId));
...
export async function changePassword(userId: string, currentPassword: string | null, newPassword: string, keepSessionId?: string) {
  ...
  await db.transaction(async (tx) => {
    await tx.update(users).set({ passwordHash: await hashPassword(newPassword), updatedAt: new Date() }).where(eq(users.id, userId));
