...
export type WeeklyPlan = { weekStart: string; studyDays: 3 | 5 | 7; studyMinutes: 10 | 20 | 30 | 45 | 60; learningDays: number; items: WeeklyPlanItem[]; preview: WeeklyPlanItem[]; history: Array<{ weekStart: string; completedSessions: number }> };
...
export function buildWeeklyPlan(input: Input): WeeklyPlan {
  ...
  return { weekStart, studyDays, studyMinutes, learningDays: input.learningDays, items, preview: input.plan === "FREE" ? items.slice(0, 3) : items, history: input.plan === "PREMIUM" ? input.history ?? [] : [] };
}
