...
| Local volume + Nginx | media storage/delivery | SELF_HOSTED_REQUIRED | when media enabled | R2 during rollback | MEDIUM | VPS disk/bandwidth | LOW | SELF-HOST |
...
| Cloudflare R2 + S3 SDK | media rollback/migration | LEGACY / MIGRATION ONLY | no after cutover | LOCAL | LOW | storage/egress | LOW-MEDIUM | REMOVE AFTER MIGRATION |
...
| Media | `MEDIA_ENABLED`, `MEDIA_STORAGE_PROVIDER`, `LOCAL_MEDIA_ROOT`, `MEDIA_SIGNING_SECRET` | provider-specific |
| Media legacy | `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `R2_ENDPOINT`; reserved `R2_PUBLIC_BASE_URL` | migration/rollback only |
...
Normal tests use fakes/PGlite and make no real provider calls. Payment and media have provider abstractions; OAuth is not required; email is suppressed in tests. SMTP lock-in is LOW because it is standard SMTP. Google is LOW because optional. payOS is MEDIUM because webhook/status semantics require mapping despite the abstraction. R2 is LOW-MEDIUM and already replaced by `MediaStorage` + LOCAL.
