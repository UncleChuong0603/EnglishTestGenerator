...
- Next.js emits the homepage canonical without a trailing slash. The homepage sitemap entry and `WebSite` URL use the same form, while Google displays the indexed homepage with `/`. Google selected the inspected homepage itself as canonical, so the screenshots do not show a canonical conflict. This does not establish why URL Inspection said “No referring sitemaps detected.”
