...
function part2(n) {
  ...
  const patterns = [
    [`When will ${name} finish the ${object}?`, `By ${time} on ${day}.`, `In the ${place}.`, `${name} works in ${department}.`, "direct_response"],
    [`Where should I take the ${object}?`, `Please leave it in the ${place}.`, `At about ${time}.`, `The ${department} team prepared it.`, "direct_response"],
    [`Who is reviewing the ${object}?`, `${name} from ${department}.`, `It is beside the ${place}.`, `The review starts at ${time}.`, "direct_response"],
    [`Why was the ${department} meeting moved?`, `${name} has a client appointment.`, `It is in the ${place}.`, `At ${time} on ${day}.`, "direct_response"],
    [`How can I request the ${object}?`, `Submit the online form to ${department}.`, `It arrived on ${day}.`, `Near the ${place}.`, "direct_response"],
    [`What did ${name} say about the ${object}?`, `It needs one final correction.`, `${name} is in ${department}.`, `Put it in the ${place}.`, "direct_response"],
    [`Could you bring the ${object} to the ${place}?`, `Of course, I'll take it there now.`, `The room opens at ${time}.`, `${name} wrote it on ${day}.`, "request"],
    [`Why don't we discuss the ${object} on ${day}?`, `That gives the ${department} team more time.`, `The ${place} is downstairs.`, `${name} discussed it yesterday.`, "suggestion"],
    [`Would you like me to send the ${object} to ${name}?`, `Yes, please copy the ${department} manager too.`, `It was printed at ${time}.`, `The ${place} has a copier.`, "offer"],
    [`The ${object} is in the ${place}, isn't it?`, `No, ${name} took it to ${department}.`, `Yes, the office closes at ${time}.`, `It was a useful meeting.`, "indirect_response"],
    [`Which room did ${name} reserve?`, `The ${place}.`, `At ${time} on ${day}.`, `For the ${department} workshop.`, "direct_response"],
    [`Didn't ${name} approve the ${object}?`, `Not yet; the ${department} director is checking it.`, `It is stored in the ${place}.`, `The approval meeting lasted an hour.`, "indirect_response"],
    [`How often does ${department} update the ${object}?`, `Every other ${day}.`, `${name} updates it.`, `In the ${place}.`, "direct_response"],
    [`Can I meet ${name} before ${time}?`, `Yes, but call the ${department} office first.`, `The ${place} is on the first floor.`, `${day} was very busy.`, "request"],
    [`Where is ${name}'s copy of the ${object}?`, `The ${department} assistant may have it.`, `It contains ${n} pages.`, `We reviewed it on ${day}.`, "indirect_response"],
  ];
  const [prompt, correct, ...rest] = patterns[n % patterns.length];
  const wrong = rest.slice(0, 2), subSkill = rest[2];
  ...
  return [
    {
      ...
      qs: [["What problem is mentioned?", `Printer ${n} is not working`, [`The ${object} has not arrived for order ${n}`, `${b} canceled meeting ${n}`, `The ${place} is closed for event ${n}`], "detail", "explicit_information"], ["What will the man do?", `Use a printer in the ${place}`, [`Call ${b} about file ${n}`, `Repair printer ${n}`, `Rewrite document ${n}`], "next_action", "next_action"], ["Where should a set of copies be left?", `At the ${other}`, [`In ${department} office ${n}`, `Beside printer ${n}`, `At the ${place} desk ${n}`], "detail", "location"]],
    },
    {
      ...
      qs: [["Why is the woman concerned?", `A delivery will arrive after a presentation`, [`Order ${n} contains the wrong items`, `${b} cannot attend presentation ${n}`, `The ${place} is unavailable for order ${n}`], "detail", "explicit_information"], ["What will the woman probably do next?", `Call ${b}`, [`Cancel order ${n}`, `Move presentation ${n}`, `Visit ${department} office ${n}`], "next_action", "next_action"], ["Where is the order now?", `At the ${place}`, [`At the ${other} for order ${n}`, `In ${department} room ${n}`, `In presentation room ${n}`], "detail", "location"]],
    },
    {
      ...
      qs: [["Why has the man most likely come to the office?", `For job interview ${n}`, [`To deliver order ${n}`, `To repair equipment ${n}`, `To reserve room ${n}`], "purpose", "purpose"], ["When will the man's appointment begin?", `At ${time}`, [`On ${day} afternoon for interview ${n}`, `After lunch for interview ${n}`, `At closing time for interview ${n}`], "detail", "explicit_information"], ["What will the woman do?", `Show the man to the ${other}`, [`Call manager ${n}`, `Take coat ${n} home`, `Conduct interview ${n}`], "next_action", "next_action"]],
    },
    {
    ...
    },
    {
      ...
      qs: [["What did the woman find?", `Incorrect figures in report ${n}`, [`Missing delivery ${n}`, `Broken printer ${n}`, `Empty meeting room ${n}`], "detail", "explicit_information"], ["Who will the man contact?", `${a}`, [`${b} about report ${n}`, `Supplier ${n}`, `Building manager ${n}`], "detail", "explicit_information"], ["Why is the corrected version needed?", `For meeting ${n} on ${day}`, [`For refund ${n}`, `For inspection ${n}`, `For course ${n}`], "purpose", "purpose"]],
    },
    {
